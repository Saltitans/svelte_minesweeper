...
class GameHandler {
    ...
    boardSize: number = $state(10);
    board: GameCell[][] = $derived(this._initializeBoard(this.boardSize));
    ...
    _displayAdjacentCells(cell: null | GameCell, previousCell: GameCell): void {
        ...
        if (effectiveCell.x > 0 && effectiveCell.y < this.boardSize - 1) this._displayAdjacentCells(this.board[effectiveCell.x - 1][effectiveCell.y + 1], effectiveCell);

        if (effectiveCell.y < this.boardSize - 1) this._displayAdjacentCells(this.board[effectiveCell.x][effectiveCell.y + 1], effectiveCell);

        if (effectiveCell.x < this.boardSize - 1 && effectiveCell.y < this.boardSize - 1) this._displayAdjacentCells(this.board[effectiveCell.x + 1][effectiveCell.y + 1], effectiveCell);

        if (effectiveCell.x < this.boardSize - 1) this._displayAdjacentCells(this.board[effectiveCell.x + 1][effectiveCell.y], effectiveCell);

        if (effectiveCell.x < this.boardSize - 1 && effectiveCell.y > 0) this._displayAdjacentCells(this.board[effectiveCell.x + 1][effectiveCell.y - 1], effectiveCell);
